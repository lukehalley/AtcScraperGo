...
func ScrapePairInfo(Network geckoterminal_types.GeckoTerminalNetworkWithDexs, NetworkStablecoins []string, Dex geckoterminal_types.Dex, DexDBId int, DexPair geckoterminal_types.PairData, BlacklistPairAddresses []string, TxsToCollect int) {
	...
	FactoryAndRouterAddressAvailable := Dex.RouterAddress != "" && Dex.FactoryAddress != ""

	// Try And Get Router Address
	if FactoryAndRouterAddressAvailable {
		...
		// Get Router Dex ABI
		if !RouterABIStored {
			Dex.RouterAbi = CollectDexsABI(Network, Dex.RouterAddress)
		} else {
		...
		// Get Factory Dex ABI
		if !FactoryABIStored {
			Dex.FactoryAbi = CollectDexsABI(Network, Dex.FactoryAddress)
		} else {
		...
		if len(Pair.Transactions) > 0 {
			...
			if (BaseTokenIsStablecoin || QuoteTokenIsStablecoin) && !BothTokensAreStablecoins {
				...
				if Dex.RouterAbi != "" {
					RouterAbi = Dex.RouterAbi
				}

				// Collect Transactions
				for _, PairTransaction := range Pair.Transactions {
					go DecodeTransaction(Network, PairTransaction, RouterAbi, TxDecodeWaitGroup, TxDecodeChannel)
				}
				...
				if len(CollectedDecodedTxs) > 0 {
					...
					if DexDBId < 0 {
						...
						if !FactoryABIStored {
							mysql_insert.AddABIToDB(Network.NetworkDBId, DexDBId, "factory", Dex.FactoryAddress, Dex.FactoryAbi, "")
						}

						// Add Router ABI To DB If Not Stored
						if !RouterABIStored {
							mysql_insert.AddABIToDB(Network.NetworkDBId, DexDBId, "router", Dex.RouterAddress, Dex.RouterAbi, "")
						}
